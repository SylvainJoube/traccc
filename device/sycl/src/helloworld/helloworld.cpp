...
void hello_world::say_hello() const {
    std::cout << "Hello world from SYCL !" << std::endl;
}

void hello_world::test_sycl_vector_sum() const {
    ...
    try {
        ...
        data_type *input_a_sycl =
            cl::sycl::malloc_host<data_type>(vector_size, sycl_q);
        data_type *input_b_sycl =
            cl::sycl::malloc_host<data_type>(vector_size, sycl_q);
        data_type *output_sycl =
            cl::sycl::malloc_host<data_type>(vector_size, sycl_q);

        // Copy from host memory to sycl memory
        sycl_q.memcpy(input_a_sycl, input_a, vector_size * sizeof(data_type))
            .wait();
        sycl_q.memcpy(input_b_sycl, input_b, vector_size * sizeof(data_type))
            .wait();

        // Kernel : vectors sum
        class SyclHelloworldKernel;

        sycl_q.parallel_for<SyclHelloworldKernel>(
            cl::sycl::range<1>(vector_size), [=](cl::sycl::id<1> cell_index) {
                auto i = cell_index.get(0);
                output_sycl[i] = input_a_sycl[i] + input_b_sycl[i];
            });

        // Copy from sycl memory to host memory
        sycl_q.memcpy(output, output_sycl, vector_size * sizeof(data_type))
            .wait();
        ...
        for (uint i = 0; i < vector_size; ++i) {
            ...
            if (output[i] != sum) {
                has_error = true;
                std::cout << "SYCL HELLOWORLD ERROR : at index " << i
                          << " expected " << sum << " but found " << output[i]
                          << "." << std::endl;
            }
        }
        ...
        std::cout << "SYCL HELLOWORLD ERROR : An exception has been caught "
                     "while processing SyCL code.\n";
    }
}
