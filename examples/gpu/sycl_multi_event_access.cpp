...
{ try {
    ...
    for (uint iwarmup_and_final_exec = 0; iwarmup_and_final_exec < warmup_count + 1; ++iwarmup_and_final_exec) {
        ...
        cl::sycl::buffer<traccc::sycl::ccl::input_module_ctrl, 1> b_ctrl_input_array(ctrl_input_array, cl::sycl::range<1>(module_count));
        cl::sycl::buffer<traccc::sycl::ccl::output_module_ctrl, 1> b_ctrl_output_array(ctrl_output_array, cl::sycl::range<1>(module_count));

        cl::sycl::buffer<traccc::sycl::ccl::input_cell, 1> b_input_cell_array(input_cell_array, cl::sycl::range<1>(total_cell_count));
        cl::sycl::buffer<traccc::sycl::ccl::output_cell, 1> b_output_cell_array(output_cell_array, cl::sycl::range<1>(total_cell_count));

        t_buffer_creation = get_ms() - t_start;


        // ==== accessors and parallel for ====
        ...
        t_start = get_ms();
        // Submitting to the SyCL queue
        sycl_q.submit([&](cl::sycl::handler &h) {
            // Setting up the accessors
            auto a_ctrl_input_array = b_ctrl_input_array.get_access<cl::sycl::access::mode::read>(h); // read
            auto a_ctrl_output_array = b_ctrl_output_array.get_access<cl::sycl::access::mode::discard_write>(h); // write
            auto a_input_cell_array = b_input_cell_array.get_access<cl::sycl::access::mode::read>(h); // read
            auto a_output_cell_array = b_output_cell_array.get_access<cl::sycl::access::mode::discard_write>(h); // write
            ...
            h.parallel_for<MyKernel_a>(cl::sycl::range<1>(module_count), [=](cl::sycl::id<1> module_indexx) {
                ...
                uint first_cindex = a_ctrl_input_array[module_index].cell_start_index;
                uint cell_count = a_ctrl_input_array[module_index].cell_count;
                ...
                for (unsigned int i=0; i < cell_count; ++i){
                    ...
                    int ai = i;
                    if (i > 0){
                        const traccc::sycl::ccl::input_cell &ci = a_input_cell_array[i + first_cindex];
                        for (unsigned int j = start_j; j < i; ++j){
                            const traccc::sycl::ccl::input_cell &cj = a_input_cell_array[j + first_cindex];
                ...
                for (unsigned int i = 0; i < cell_count; ++i){
                    a_output_cell_array[i + first_cindex].label = L[i];
                }
                a_ctrl_output_array[module_index].cluster_count = labels;
            });
        });
        ...
        b_ctrl_output_array.get_access<cl::sycl::access::mode::read>();
        b_output_cell_array.get_access<cl::sycl::access::mode::read>();
        sycl_q.wait_and_throw();
        ...
        t_start = get_ms();
        } // out of scope buffers and accessors, no no segfault
        ...
        t_gpu = t_linearization_allocation + t_linearization_fill
                ...
                + t_free_linearization + t_buffer_creation;
        std::cout 
                ...
                << "t_allocation_device          = ..." << std::endl // << t_allocation_device << std::endl
                << "t_copy_to_device - - - - - - = " << t_copy_to_device << std::endl
                << "t_buffer_creation - - - - -  = " << t_buffer_creation << std::endl
                << "t_parallel_for               = " << t_parallel_for << std::endl
                << "t_read_from_device - - - - - = " << t_read_from_device << std::endl
                << "t_free_device                = " << t_free_device << " (out of scope)" << std::endl
